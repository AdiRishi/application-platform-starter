...
flowchart TD
  ...
  API -->|live progress RPC| Processor[Processor Worker]
  API --> D1[(D1)]
  API --> R2[(R2)]
  API --> Queue[Profile jobs]
  Queue --> Processor
  Processor -->|source and result RPC| API
...
The processor binds the API in the same way. `platform/artifact-client.ts` gives source and result calls a five-second deadline and maps failures into retryable processing errors. The API exposes `startProfile`, `getProfileSource`, `completeProfile`, and `failProfile` alongside its read methods. Worker declarations and implementation layers allow bindings in both directions.
...
The API exclusively owns the artifact database and source bucket. The processor consumes queue messages and calls the API to start a profile, fetch source bytes, and report completion or failure. It has no D1 or R2 binding. Sources are capped at 256 KB and travel as bytes through native RPC.
