...
Pure Effect tests and UI component tests stay in their owning workspaces. The web workspace uses separate Vitest projects for Node tests and component tests in Chromium. Component tests use `@vitest/browser-playwright`, `vitest-browser-react`, and the application's styles. `pnpm test:unit` runs these tests and the script tests without starting infrastructure. CI installs Chromium and uses this command. Test tasks run independently of dependency workspaces' tests. Run `pnpm --filter @repo/infra test` for the local platform tests or `pnpm --filter @repo/web test` for the web tests. Vitest runs teardown hooks in registration order so stack destruction precedes Alchemy's runtime cleanup.

Only the public application suite starts the website. API, processor, dispatch, migration, and profile-session suites deploy their own smaller stacks. Readiness checks retry transient transport errors and HTTP 404/5xx responses for at most 30 seconds; exhausted retries fail the suite. Browser assertions wait for the expected UI state within their timeout; entire tests are not retried.

The public application suite invokes Playwright Test after Alchemy deploys the stack. It passes `APPLICATION_URL` to the child process and fails if Playwright exits unsuccessfully. `infra/tests/alchemy.run.browser.test.ts` checks the upload, profile, source download, and missing-profile flow. Playwright owns the browser and page fixtures; Alchemy's teardown destroys the stack even if the browser test fails. Playwright saves failure traces and screenshots under `infra/test-results/`. Local and live application tests use the same browser suite.

Install Chromium once with `pnpm --filter @repo/web exec playwright install chromium`. Both browser runners use the same pinned Playwright version and browser installation. Use the live suite when you change infrastructure or behavior that depends on a real deployment:
